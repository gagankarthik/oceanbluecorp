  ...
  return (
    <div className="rounded-xl overflow-hidden border border-slate-800 my-4">
      <div className="flex items-center justify-between bg-slate-900 px-4 py-2">
        <span className="text-[10px] text-slate-400 font-mono uppercase tracking-wider">{lang}</span>
        <button
          onClick={() => {
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden mb-3 hover:border-slate-300 transition-colors">
      ...
      {open && (
        <div className="border-t border-slate-100 px-5 py-5 bg-slate-50/50 space-y-5">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      <th className="py-2.5 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Name</th>
                      <th className="py-2.5 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Type</th>
                      <th className="py-2.5 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Required</th>
                      <th className="py-2.5 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Description</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {params.map((p) => (
                      <tr key={p.name}>
                        ...
                        <td className="py-2.5 px-4"><code className="text-xs font-mono text-slate-500">{p.type}</code></td>
                        <td className="py-2.5 px-4">
                          {p.required
                            ? <span className="text-[10px] font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">required</span>
                            : <span className="text-[10px] text-slate-400">optional</span>}
                        </td>
                        <td className="py-2.5 px-4 text-sm text-slate-600">{p.description}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {responseExample && (
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Example Response</p>
...
export default function DevelopersContent() {
  return (
    <div className="horizon min-h-screen bg-white">
      {/* Hero */}
      <div className="text-white" style={{ background: "linear-gradient(135deg, #050912 0%, #0a1730 45%, #07142b 100%)" }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8 py-20">
          <div className="flex items-center gap-2 text-[var(--hz-cyan-400)] text-sm font-medium mb-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-sm">
              <Globe className="w-3.5 h-3.5 text-[var(--hz-cyan-400)]" />
              REST / JSON
            </div>
          </div>
        </div>
            ...
            <nav className="space-y-0.5">
              {NAV.map((item) => (
                <a
                  key={item.id}
                  href={`#${item.id}`}
                  className="block text-sm text-slate-500 hover:text-[var(--hz-cobalt)] py-1 transition-colors"
                >
                  {item.label}
                </a>
              ))}
            </nav>
            <div className="mt-8 p-4 bg-[var(--hz-cobalt-100)] rounded-xl border border-[var(--hz-cobalt-100)]">
          ...
          <Section id="overview" title="Overview">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={item.title} className="flex gap-3 p-4 border border-slate-200 rounded-xl">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Status</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Code</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Meaning</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  <tr><td className="py-3 px-4"><Badge label="401" color="bg-rose-100 text-rose-700" /></td><td className="py-3 px-4 font-mono text-xs text-slate-600">Missing API key</td><td className="py-3 px-4 text-slate-600">No X-API-Key header provided</td></tr>
                  <tr><td className="py-3 px-4"><Badge label="401" color="bg-rose-100 text-rose-700" /></td><td className="py-3 px-4 font-mono text-xs text-slate-600">Invalid API key</td><td className="py-3 px-4 text-slate-600">Key not found in our system</td></tr>
                  <tr><td className="py-3 px-4"><Badge label="403" color="bg-orange-100 text-orange-700" /></td><td className="py-3 px-4 font-mono text-xs text-slate-600">Key disabled</td><td className="py-3 px-4 text-slate-600">Key has been revoked or disabled</td></tr>
                </tbody>
              </table>
            </div>
          </Section>
...
GET /api/v1/jobs/b3f1a2c4-1234-5678-abcd-ef0123456789`}</CodeBlock>
            ...
            <div className="mt-4 overflow-x-auto rounded-xl border border-slate-200">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Field</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Values</th>
                  </tr>
                </thead>
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Field</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Type</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Notes</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {[
                    ...
                    <tr key={field} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-3 px-4"><code className="text-xs font-mono text-slate-500">{type}</code></td>
                      <td className="py-3 px-4 text-xs text-slate-600">{notes}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  "error": "Missing API key. Pass X-API-Key header."
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200 mt-4">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">HTTP Code</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Meaning</th>
                    <th className="py-3 px-4 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Action</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {[
                    ...
                    <tr key={code} className="hover:bg-slate-50/60">
                      ...
                      <td className="py-3 px-4 text-sm text-slate-700">{meaning}</td>
                      <td className="py-3 px-4 text-sm text-slate-500">{action}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Section id="quickstart" title="Quickstart">
            <p className="text-slate-600 leading-relaxed mb-4">Copy-paste examples to get up and running in minutes.</p>

            <p className="text-sm font-semibold text-slate-700 mb-2">cURL</p>
            <CodeBlock lang="bash">{`curl https://oceanbluecorp.com/api/v1/jobs \\
  -H "X-API-Key: obk_live_your_key_here" | jq .`}</CodeBlock>

            <p className="text-sm font-semibold text-slate-700 mb-2 mt-5">JavaScript / TypeScript</p>
...
async function getJob(id: string) {
  ...
  return res.json(); // { data: Job }
            ...
            <p className="text-sm font-semibold text-slate-700 mb-2 mt-5">Python</p>
          ...
          <Section id="get-access" title="Get Access">
            <div className="bg-gradient-to-br from-[var(--hz-cobalt-100)] to-slate-50 border border-[var(--hz-cobalt-100)] rounded-2xl p-8 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <Link
                  href="/careers"
                  className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 hover:border-slate-300 text-slate-700 text-sm font-semibold rounded-xl transition-colors"
                >
                  View Open Positions
                </Link>
              </div>
